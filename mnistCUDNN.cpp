...
const char *fourth_image = "six_28x28.pgm";
const char *fifth_image  = "eight_28x28.pgm";
...
{   
    ...
    if (argc == 1 || (argc == 2) && checkCmdLineFlag(argc, (const char **)argv, "device"))
    {
        ...
        }
        {
            ...
            if (i1 != 1 || i2 != 3 || i3 != 5)
            {
                std::cout << "\nBaseline test failed! (1,3,5)\n";
                FatalError("Prediction mismatch");
            }
            else
            {
                std::cout << "\nBaseline test passed! (1,3,5)\n";
            }
        }
        ...
        {
            std::cout << "\n========== MNIST Evaluation ==========\n";
            network_t<float> mnist_eval;
            Layer_t<float> conv1(1,20,5,conv1_bin,conv1_bias_bin,argv[0]);
            Layer_t<float> conv2(20,50,5,conv2_bin,conv2_bias_bin,argv[0]);
            Layer_t<float>   ip1(800,500,1,ip1_bin,ip1_bias_bin,argv[0]);
            Layer_t<float>   ip2(500,10,1,ip2_bin,ip2_bias_bin,argv[0]);

            // data/output/ 에서 PGM 파일 목록 읽기
            std::vector<std::string> pgm_files;
            DIR *dir = opendir("data/output");
            if (dir)
            {
                struct dirent *ent;
                while ((ent = readdir(dir)) != NULL)
                {
                    ...
                    if (name.size() > 4 && name.substr(name.size()-4) == ".pgm")
                        pgm_files.push_back("data/output/" + name);
                }
                closedir(dir);
            ...
            if (pgm_files.empty())
            {
                std::cout << "No PGM files found in data/output/\n";
            }
            else
            {
                ...
                for (auto& pgm : pgm_files)
                {
                    std::string fname = pgm.substr(pgm.find_last_of("/")+1);
                    std::cout << "INPUT: " << fname << "\n";

                    auto t0  = std::chrono::high_resolution_clock::now();
                    int pred = mnist_eval.classify_example(pgm.c_str(), conv1, conv2, ip1, ip2);
                    checkCudaErrors(cudaDeviceSynchronize());
                    ...
                    std::cout << "Result of classification: " << pred << "\n\n";
                }

                std::cout << "Total Images : " << count << "\n";
                std::cout << "Total Time   : " << std::fixed << std::setprecision(3)
                          << total_ms / 1000.0 << " sec\n\n";

                for (int d = 0; d < 10; d++)
                    std::cout << "Digit " << d << " : " << digit_count[d] << "\n";
            }
            std::cout << "======================================\n";
        }
